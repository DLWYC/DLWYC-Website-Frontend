...
const EventCard = (e) =>{
     return(
     <div className={`border border-[#b1b1b131] bg-white basis-[50%] lg:h-[50vh] p-3 rounded-lg lg:space-x-3 ${e.position}`}>
          <div className={`basis-[40%] rounded-xl bg-no-repeat bg-cover bg-center overflow-hidden lg:h-full`} >
          {/* <div className={`basis-[40%] rounded-xl bg-no-repeat bg-cover bg-center border border-red-500 h-[30lvh] ${e.imagesize === "True" & e.id === 1 ? 'lg:h-[80lvh]' : 'lg:h-full'}`} style={{backgroundImage: `url(${e.image})`}} >
               ...
               {/* <Link to={e.link} className="border border-orange-400 w-full h-full"> */}
                    <img src={e.image} alt="" className={`w-full border-none outline-none object-cover object-top h-[450px] ${e.imagesize === "True" & e.id === 1 ? 'lg:h-[80lvh]' : 'lg:h-full'} `} />
               {/* </Link> */}
          </div>
