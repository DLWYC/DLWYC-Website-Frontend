...
function SignUpView() {
  ...
  const availableParishes = useMemo(() => {
    ...
    const selected = Churches.find(
      (c) => c.archdeaconry === formData.archdeaconry,
    );
  ...
  const validate = () => {
    ...
    if (!formData.profession) next.profession = "Select A Role";
    setErrors(next);
        ...
        <div className="max-w-lg w-full space-y-4">
          ...
          <div className="">
            ...
            <div>
              ...
              <InputWrapper icon={Church} error={errors.profession}>
                <select
                  name="profession"
                ...
                >
                  ...
                  {Professions.map((profession) => (
                    <option key={profession.id} value={profession.role}>
                      {profession.role}
                    </option>
                  ))}
                </select>
            ...
            <button
              ...
              disabled={isPending}
              className="w-full bg-primary-main hover:bg-reddish active:scale-[0.98] text-white py-2.5 rounded-lg text-[14px] font-[400] transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-4"
            >
              {isPending ? (
                <>
            ...
            <p className="text-sm text-center mt-2 text-gray-500">
              ...
              <Link
                to="/login"
                className="text-reddish font-medium hover:underline"
              >
                Sign in
              </Link>
            </p>
